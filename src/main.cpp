...
uint256 hashGenesisBlock("0x00000a3cfbcfb461af68cc5c9d0a0bf9b145ef70c4581fa1c45d05b8c57817cc"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LemurCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LemurCoin Signed Message:\n";
...
{
    // LemurCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 86400; // lemurcoin 1 day
static const int64 nTargetSpacing = 300; // lemurcoins every 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LemurCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, lemurcoin - DarkGravity v3, written by Evan Duffield - evan@lemurcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LemurCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LemurCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000a4df98f38f15a6dcbad0c24d3bf129dfb94e3fa4ab7b4088464766393eb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Lemurized summer time!!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407776787;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1071994;

        if (fTestNet)
        {
            block.nTime    = 1406479800;
            block.nNonce   = 1359021;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3fc7f217982a3ce6e136517739828e02748fa1073e05a7bd05c5380979272dec"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // LemurCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LemurCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LemurCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LemurCoinMiner(CWallet *pwallet)
{
    printf("LemurCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lemurcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LemurCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LemurCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LemurCoinMiner, pwallet));
}
